# Dog Companion - Instruções para Claude
...
#### `BaseAbilityCard`
Para habilidades fixas/automáticas de níveis 1 e 2.
```tsx
<BaseAbilityCard
  level={1}
...
#### `AbilityCard`
Para exibir habilidades já selecionadas (na aba de Combate).
```tsx
<AbilityCard
  name="Nome"
...
Para cards clicáveis de seleção de habilidades.
```tsx
<AbilitySelectionCard
  title="Nome – Habilidade"
  description="Descrição"
